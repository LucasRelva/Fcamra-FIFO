function JogosClass() {
  ...
  this.init = async function(unity_id, idContainer) {
      ...
      this.loadBoxes(unity_id,idContainer);
      this.loadSlick();
  };
  
  this.loadJogos = async function (unity_id) {
  ...
  this.loadBoxes = function (unity_id,idContainer) {
    ...
    if(this.jogos && container){
        for (const game of this.jogos) {
            ...
            link.setAttribute("href", `jogos.html?unity_id=${unity_id}&game_id=${game.id}`); // adiciona ao botão o link do game
  ...
  this.getGameInfos = async function (unity_id,game_id) {
  };
}
